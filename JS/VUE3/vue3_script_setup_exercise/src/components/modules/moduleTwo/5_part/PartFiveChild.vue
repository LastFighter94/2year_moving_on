...
// watch(x, (newX) => {
//     logTs(`value of x is ${newX}`)
// })
//
// watch(y, (newY) => {
//     logTs(`value of y is ${newY}`)
// })
//
// watch([x, y], ([newX, newY]) => {
//     logTs(`values of x and y are x: ${newX}, y: ${newY}`)
// })
//
// watch(
//     () => x.value + y.value,
//     (sum) => {
//         logTs(`sum value is ${sum}`)
//     }
// )
...
//     return y.value + 'test'
// }], ([newX, newY]) => {
//     logTs(`value of x is ${newX} and y is ${newY}`)
// })
...
watch(
    () => obj.count.value,
    (newCount, oldCount) => {
        logTs(`new count value is ${newCount}, old is ${oldCount}`)
    },
    { deep: true }
)
...
// watchEffect(async () => {
    onInvalidate(() => {
        logTs('срабатывает до того как выполнится код ниже !!!')
    })

    logTs(`${count.value}, watchEffect`)
    document.getElementById('testCountId') && logTs(document.getElementById('testCountId').innerText)
    let response = await fetch(`https://jsonplaceholder.typicode.com/todos/${todoId.value}`)

    // logTs(`${count.value}, watchEffect`) - will not work because - await, async - should be passed before async logic
    data.value = await response.json()
})
...
const watcher = watchEffect(() => {
    logTs(`${notes.value}, UNWATCH EXAMPLE`)
    logTs(`${notesArePublic.value}, UNWATCH EXAMPLE`)
})
...
//         logTs(`${value} value, oldValue ${oldValue}`)
//         onInvalidate(() => {
//             logTs('something')
//         });
//     },
