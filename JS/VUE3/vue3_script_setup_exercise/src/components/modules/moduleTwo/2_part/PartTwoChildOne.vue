...
const liClassName = (p: IPerson , index: Number): string => {
...
const toggleManClass = (p: IPerson): void => {
    p.man = !p.man
}

let inputValue = ref<string>('InputValue')

let badWords: string[] = ['shit', 'bitch', 'fuck', 'ass']

const inputValueValidateFn = computed<string>(() => {
    let badConditionsArr: boolean[] = [
...
let radioValue = ref<string>('value radio one')
let selectValue = ref<string>('text 1 value')
let checkboxValue = ref<boolean>(true)

let selectedPeopleArr = ref<string[]>([])

let isBtnDisabled = ref<boolean>(true)
let hrefDynamicStrPart = ref<string>('google.com')
</script>
