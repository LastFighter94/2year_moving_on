<template>
    <div>
                ...
                <div v-if="group.groups === undefined && group.formFields !== undefined" class="fields">
                    <div v-for="field in group.formFields" class="field">
                        ...
                        <section v-if="currentRow">
                            <input type="text" v-model="currentRow[field.columntablekey]" :disabled="field.columntablekey === 'date'"/>
                        </section>
                    </div>
                </div>
        ...
        <div class="groups column" v-else>
            <div class="files">
                <form id="form" @submit.prevent="submitFormData" method="post" enctype="multipart/form-data">
...
export default {
    ...
    async created(){
        ...
        let {table_name: parentTableName} = await API({requestParams: this.$route.params.id}, 'get-parent-table-name','allModules', 'GET')
